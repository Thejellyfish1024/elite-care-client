...
const UpcomingCampsTable = ({ camp , refetch}) => {
    // const [open, setOpen] = useState(false);
    // const handleOpen = () => setOpen(true);
    const axiosSecure = useAxiosSecure();

    const { data } = useUpcomingRegistrationCount(camp?._id);
    const {data : professionals} = useProfessionalCount(camp?._id)
    // console.log(data);
    ...
    const handleDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                await axiosSecure.delete(`/upcoming-camps/${camp?._id}`)
                    .then(res => {
    ...
    return (
        <>
            <StyledTableRow>
                ...
                <StyledTableCell align="right" sx={{ textAlign: 'center' }}>{data?.totalUpcomingRegistration}</StyledTableCell>
                <StyledTableCell align="right" sx={{ textAlign: 'center' }}>{professionals?.totalProfessionals}</StyledTableCell>
